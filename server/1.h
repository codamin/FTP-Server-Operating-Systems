...
IP addresses and ports for just a tad so we have that sorted out. Then
we'll talk about how the sockets API stores and manipulates IP addresses
and other data.
...
Gloom, Vint Cerf <http://en.wikipedia.org/wiki/Vinton_Cerf> is also
well-known for being The Father Of The Internet. So I really am in no
...
And so, IPv6 was born. Since Vint Cerf is probably immortal (even if his
physical form should pass on, heaven forbid, he is probably already
existing as some kind of hyper-intelligent ELIZA
<http://en.wikipedia.org/wiki/ELIZA> program out in the depths of the
...
many, but /79 MILLION BILLION TRILLION times as many possible
...
(for real, 2^128 ). That's like a million IPv4 Internets for /every
...
colon, like this: 2001:0db8:c9d2:aee5:73e3:934a:a5ae:9551.
...
The address ::1 is the /loopback address/. It always means "this machine
I'm running on now". In IPv4, the loopback address is 127.0.0.1.
...
galaxy has parking meters. You know it's true.


        3.1.1. Subnets

For organizational reasons, it's sometimes convenient to declare that
"this first part of this IP address up through this bit is the /network
portion/ of the IP address, and the remainder is the /host portion/.
...
the /netmask/, which you bitwise-AND with the IP address to get the
network number out of it. The netmask usually looks something like
255.255.255.0. (E.g. with that netmask, if your IP is 192.0.2.12, then
your network is 192.0.2.12 AND 255.255.255.0 which gives 192.0.2.0.)
...
(Note that the netmask is /ALWAYS/ a bunch of 1-bits followed by a bunch
of 0-bits.)

But it's a bit unwieldy to use a big string of numbers like 255.192.0.0
...
coincidentally, by UDP (datagram sockets). It is the /port number/. It's
...
numbers. You can see them all in the Big IANA Port List
<http://www.iana.org/assignments/port-numbers> or, if you're on a Unix
box, in your //etc/services/ file. HTTP (the web) is port 80, telnet is
port 23, SMTP is port 25, the game DOOM
<http://en.wikipedia.org/wiki/Doom_(video_game)> used port 666, etc. and
so on. Ports under 1024 are often considered special, and usually
require special OS privileges to use.
...
By Order of the Realm! There shall be two byte orderings, hereafter to
be known as Lame and Magnificent!
...
it in two sequential bytes b3 followed by 4f. Makes sense, and, as
Wilford Brimley <http://en.wikipedia.org/wiki/Wilford_Brimley> would
tell you, it's the Right Thing To Do. This number, stored with the big
end first, is called /Big-Endian/.

Unfortunately, a few computers scattered here and there throughout the
...
(two bytes) and long (four bytes). These functions work for the unsigned
variations as well. Say you want to convert a short from Host Byte Order
to Network Byte Order. Start with "h" for "host", follow it with "to",
then "n" for "network", and "s" for "short": h-to-n-s, or *htons()*
...
point, check out the section on Serialization <#serialization>, far below.
...
lookups. That'll make more sense later when we get to actual usage, but
just know for now that it's one of the first things you'll call when
making a connection.
...
call to *getaddrinfo()* to fill out your struct addrinfo for you is all
you'll need. You /will/, however, have to peer inside these structs to
...
Some structs are IPv4, some are IPv6, and some are both. I'll make notes
of which are what.
...
*connect()* wants a struct sockaddr*, you can still use a struct
...
Finally, the /sin_port/ must be in /Network Byte Order/ (by using
*htons()*!)

Let's dig deeper! You see the /sin_addr/ field is a struct in_addr. What
is that thing? Well, not to be overly dramatic, but it's one of the
...
Whoa! Well, it /used/ to be a union, but now those days seem to be gone.
Good riddance. So if you have declared /ina/ to be of type struct
sockaddr_in, then /ina.sin_addr.s_addr/ references the 4-byte IP address
(in Network Byte Order). Note that even if your system still uses the
God-awful union for struct in_addr, you can still reference the 4-byte
IP address in exactly the same way as I did above (this due to #defines.)
...
address "10.12.110.57" or "2001:db8:63b3:1::3490" that you want to store
into it. The function you want to use, *inet_pton()*, converts an IP
...
("pton" stands for "presentation to network"—you can call it "printable
to network" if that's easier to remember.) The conversion can be made as
follows:
...
(Or a struct in6_addr that you want in, uh, "hex-and-colons" notation.)
In this case, you'll want to use the function *inet_ntop()* ("ntop"
means "network to presentation"—you can call it "network to printable"
...
the historical function to do this conversion was called *inet_ntoa()*.
It's also obsolete and won't work with IPv6.)

Lastly, these functions only work with numeric IP addresses—they won't
do any nameserver DNS lookup on a hostname, like "www.example.com". You
will use *getaddrinfo()* to do that, as you'll see later on.


        3.4.1. Private (Or Disconnected) Networks

Lots of places have a firewall that hides the network from the rest of
the world for their own protection. And often times, the firewall
translates "internal" IP addresses to "external" (that everyone else in
the world knows) IP addresses using a process called /Network Address
Translation/, or NAT.
...
10./x/./x/./x/ is one of a few reserved networks that are only to be
used either on fully disconnected networks, or on networks that are
behind firewalls. The details of which private network numbers are
available for you to use are outlined in RFC 1918
<http://tools.ietf.org/html/rfc1918>, but some common ones you'll see
are 10./x/./x/./x/ and 192.168./x/./x/, where /x/ is 0-255, generally.
...
IPv6 has private networks, too, in a sense. They'll start with fd/xx/:
(or maybe in the future fc/XX/:), as per RFC 4193
<http://tools.ietf.org/html/rfc4193>. NAT and IPv6 don't generally mix,
however (unless you're doing the IPv6 to IPv4 gateway thing which is
beyond the scope of this document)—in theory you'll have so many
addresses at your disposal that you won't need to use NAT any longer.
But if you want to allocate addresses for yourself on a network that
...
this, but this is what applies to the guide.)
 ...
 1. First of all, try to use *getaddrinfo()* <#getaddrinfo> to get all
    the struct sockaddr info, instead of packing the structures by hand.
    This will keep you IP version-agnostic, and will eliminate many of
    the subsequent steps.
 2. Any place that you find you're hard-coding anything related to the
    IP version, try to wrap up in a helper function.
 3. Change AF_INET to AF_INET6.
 4. Change PF_INET to PF_INET6.
 5.

    Change INADDR_ANY assignments to in6addr_any assignments, which are
    slightly different:
 ...
 6. Instead of struct sockaddr_in use struct sockaddr_in6, being sure to
    add "6" to the fields as appropriate (see structs <#structs>,
    above). There is no /sin6_zero/ field.
 7. Instead of struct in_addr use struct in6_addr, being sure to add "6"
    to the fields as appropriate (see structs <#structs>, above).
...
lay out the system calls in the following sections in /exactly/
...
/(Please note that for brevity, many code snippets below do not include
...
stand-alone programs, though, so use those as a model.)/


      5.1. *getaddrinfo()*—Prepare to launch!

This is a real workhorse of a function with a lot of options, but usage
is actually pretty simple. It helps set up the structs you need later on.
...
information by hand into a struct sockaddr_in, and use that in your calls.
...
out the structs you need, besides!
...
or the name of a particular service (found in The IANA Port List
<http://www.iana.org/assignments/port-numbers> or the //etc/services/
file on your Unix machine) like "http" or "ftp" or "telnet" or "smtp" or
whatever.
...
listening or network setup; it merely sets up structures we'll use later:
...
don't care if we use IPv4 or IPv6. You can set it to AF_INET or AF_INET6
if you want one or the other specifically.
...
information. This short program
<http://beej.us/guide/bgnet/examples/showip.c> will print the IP
...
different types of struct sockaddrs depending on the IP version. Sorry
...
still do that. (/domain/ is PF_INET or PF_INET6, /type/ is SOCK_STREAM
or SOCK_DGRAM, and /protocol/ can be set to 0 to choose the proper
protocol for the given type. Or you can call *getprotobyname()* to look
up the protocol you want, "tcp" or "udp".)

(This PF_INET thing is a close relative of the AF_INET that you can use
when initializing the /sin_family/ field in your struct sockaddr_in. In
fact, they're so closely related that they actually have the same value,
and many programmers will call *socket()* and pass AF_INET as the first
argument instead of *PF_INET*. Now, get some milk and cookies, because
it's times for a story. Once upon a time, a long time ago, it was
thought that maybe an address family (what the "AF" in "AF_INET" stands
for) might support several protocols that were referred to by their
protocol family (what the "PF" in "PF_INET" stands for). That didn't
happen. And they all lived happily ever after, The End. So the most
correct thing to do is to use AF_INET in your struct sockaddr_in and
...
// See the section on client/server <#clientserver> for real examples.]
...
*listen()* for incoming connections on a specific port—multiplayer
network games do this when they tell you to "connect to 192.168.5.10
port 3490".) The port number is used by the kernel to match an incoming
packet to a certain process's socket descriptor. If you're going to only
be doing a *connect()* (because you're the client, not the server), this
is probably be unnecessary. Read it anyway, just for kicks.
...
By using the AI_PASSIVE flag, I'm telling the program to bind to the IP
of the host it's running on. If you want to bind to a specific local IP
address, drop the AI_PASSIVE and put an IP address in for the first
...
your struct sockaddr_in6. (There is also a macro IN6ADDR_ANY_INIT that
you can use in a variable initializer.)
...
(unless you're the superuser)! You can have any port number above that,
right up to 65535 (provided they aren't already being used by another
program.)
...
little bit of a socket that was connected is still hanging around in the
kernel, and it's hogging the port. You can either wait for it to clear
(a minute or so), or add code to your program allowing it to reuse the
port, like this:
...
*telnet* where you only care about the remote port), you can simply call
*connect()*, it'll check to see if the socket is unbound, and will
*bind()* it to an unused local port if necessary.
...
you to connect to "10.12.110.57" on port "23" (the standard telnet
...
Again, old-school programs filled out their own struct sockaddr_ins to
...
queue. What does that mean? Well, incoming connections are going to wait
in this queue until you *accept()* them (see below) and this is the
limit on how many can queue up. Most systems silently limit this number
...
sockaddr_storage) before its address is passed to *accept()*. *accept()*
will not put more than that many bytes into /addr/. If it puts fewer in,
it'll change the value of /addrlen/ to reflect that.
...
less than the number you told it to send!/ See, sometimes you tell it to
send a whole gob of data and it just can't handle it. It'll fire off as
much of the data as it can, and trust you to send the rest later.
...
this: if the packet is small (less than 1K or so) it will /probably/
manage to send the whole thing all in one go. Again, -1 is returned on
error, and /errno/ is set to the error number.
...
side has closed the connection on you! A return value of 0 is *recv()*'s
way of letting you know this has occurred.
...
leave me with unconnected datagram sockets?" No problemo, amigo. We have
just the thing.
...
to a struct sockaddr (which will probably be another struct sockaddr_in
or struct sockaddr_in6 or struct sockaddr_storage that you cast at the
last minute) which contains the destination IP address and port.
/tolen/, an int deep-down, can simply be set to sizeof *to or
...
a pointer to an int, that should be initialized to sizeof *addr or
sizeof(struct sockaddr).

The function returns -1 on error and sets /errno/ accordingly.

Once you have their address, you can use *inet_ntop()*, *getnameinfo()*,
or *gethostbyaddr()* to print or get more information. No, you can't get
...
document. Check out RFC 1413 <http://tools.ietf.org/html/rfc1413> for
more info.)
...
Take *telnet*, for instance. When you connect to a remote host on port
23 with telnet (the client), a program on that host (called *telnetd*,
the server) springs to life. It handles the incoming telnet connection,
...
handle multiple clients using *fork()*. The basic routine is: server
will wait for a connection, *accept()* it, and *fork()* a child process
...
(Also, this whole *sigaction()* thing might be new to you—that's ok. The
code that's there is responsible for reaping zombie processes that
appear as the *fork()*ed child processes exit. If you make lots of
...
we're in the datagram section of the document. Let's say that *talker*
calls *connect()* and specifies the *listener*'s address. From that
...
run *listener*, above, it just sits there until a packet arrives. What
...
and there's no data there, it's not allowed to block—it will return -1
and /errno/ will be set to EAGAIN or EWOULDBLOCK.

(Wait—it can return EAGAIN /or/ EWOULDBLOCK? Which do you check for? The
...
This being said, in modern times *select()*, though very portable, is
one of the slowest methods for monitoring sockets. One possible
alternative is libevent <http://www.monkey.org/~provos/libevent/>, or
something similar, that encapsulates all the system-dependent stuff
involved with getting socket notifications.
...
the file descriptors 0 and /sockfd/ to the set /readfds/. The parameter
/numfds/ should be set to the values of the highest file descriptor plus
one. In this example, it should be set to /sockfd+1/, since it is
...
the number of microseconds to wait. Yes, that's /micro/seconds, not
...
<http://beej.us/guide/bgnet/examples/select.c> waits 2.5 seconds for
...
Some Unices update the time in your struct timeval to reflect the amount
of time still remaining before a timeout. But others do not. Don't rely
...
to read". When you actually do *recv()* from it, *recv()* will return 0.
...
This program <http://beej.us/guide/bgnet/examples/selectserver.c> acts
like a simple multi-user chat server. Start it running in one window,
then *telnet* to it ("*telnet hostname 9034*") from multiple other
windows. When you type something in one *telnet* session, it should
...
called *poll()* which behaves much the same way *select()* does, but
with a different system for managing the file descriptor sets. Check it
out! <#pollman>
...
The function returns -1 on error (and /errno/ is still set from the call
to *send()*.) Also, the number of bytes actually sent is returned in
/len/. This will be the same number of bytes you asked it to send,
...
the donkeys. You probably have to /encapsulate/ (remember that from the
data encapsulation section <#lowlevel> way back there at the beginning?)
Read on for details!
...
socket so it errors with EWOULDBLOCK (which you can just safely ignore
if it occurs). See the *fcntl()* reference page <#fcntlman> for more
info on setting a socket to non-blocking.


      7.4. Serialization—How to Pack Data

It's easy enough to send text data across the network, you're finding,
but what happens if you want to send some "binary" data like ints or
floats? It turns out you have a few options.
...
non-bandwidth-intensive situation, such as with Internet Relay Chat
(IRC) <http://en.wikipedia.org/wiki/Internet_Relay_Chat>. However, it
has the disadvantage that it is slow to convert, and the results almost
always take up more space than the original number!

Method two: passing the raw data. This one is quite easy (but
dangerous!): just take a pointer to the data to send, and call send with it.
...
architectures represent a double (or int for that matter) with the same
bit representation or even the same byte ordering! The code is decidedly
non-portable. (Hey—maybe you don't need portability, in which case this
is nice and fast.)
...
Unfortunately, there are no similar functions for float types. Is all
hope lost?
...
Byte Order. To reverse (unencode) the number, the receiver calls *ntohs()*.
...
something quick and dirty with plenty of room for improvement:
...
The above code is sort of a naive implementation that stores a float in
a 32-bit number. The high bit (31) is used to store the sign of the
number ("1" means negative), and the next seven bits (30-16) are used to
store the whole number portion of the float. Finally, the remaining bits
(15-0) are used to store the fractional portion of the number.
...
numbers is known as IEEE-754 <http://en.wikipedia.org/wiki/IEEE_754>.
Most computers use this format internally for doing floating point math,
so in those cases, strictly speaking, conversion wouldn't need to be
done. But if you want your source code to be portable, that's an
...
<http://beej.us/guide/bgnet/examples/ieee754.c>. (Mostly—it doesn't
...
each field independently and then unpack them into the struct when they
...
<http://cm.bell-labs.com/cm/cs/tpop/>" by Kernighan and Pike, they
implement *printf()*-like functions called *pack()* and *unpack()* that
do exactly this. I'd link to them, but apparently those functions aren't
...
At this point, I'm going to drop a pointer to the BSD-licensed Typed
Parameter Language C API <http://tpl.sourceforge.net/> which I've never
used, but looks completely respectable. Python and Perl programmers will
want to check out their language's *pack()* and *unpack()* functions for
accomplishing the same thing. And Java has a big-ol' Serializable
...
build the packets. Here's a version I cooked up
<http://beej.us/guide/bgnet/examples/pack2.c> on my own based on that
which hopefully will be enough to give you an idea of how such a thing
can work.
...
<http://beej.us/guide/bgnet/examples/pack2.c> of the above code that
packs some data into /buf/ and then unpacks it into variables. Note that
when calling *unpack()* with a string argument (format specifier "s"),
it's wise to put a maximum length count in front of it to prevent a
buffer overrun, e.g. "96s". Be wary when unpacking data you get over the
...
Fortunately, RFC 4506 <http://tools.ietf.org/html/rfc4506>, the External
Data Representation Standard, already defines binary formats for a bunch
of different types, like floating point types, integer types, arrays,
raw data, etc. I suggest conforming to that if you're going to roll the
...
uses SOCK_STREAMs. When a user types ("says") something, two pieces of
...
just call the *sendall()* we implemented, above <#sendall>. But that
...
starting to define a /protocol/ that describes how a client and server
communicate!

In this case, let's assume the user name is a fixed length of 8
characters, padded with '\0'. And then let's assume the data is variable
length, up to a maximum of 128 characters. Let's have a look a sample
packet structure that we might use in this situation:
...
packet (like maybe we receive "18 42 65 6E 6A" from Benjamin, above, but
that's all we get in this call to *recv()*). We need to call *recv()*
...
packet length. Then once you have that, you can call it again specifying
exactly the remaining length of the packet (possibly repeatedly to get
all the data) until you have the complete packet. The advantage of this
method is that you only need a buffer large enough for one packet, while
the disadvantage is that you need to call *recv()* at least twice to get
all the data.
...
hold /two/ packets—in case this happened!)
...
need practice and pretty soon it'll come to you naturally. By Excalibur
I swear it!
...
authority, send data to multiple hosts /at the same time/!

With UDP (only UDP, not TCP) and standard IPv4, this is done through a
mechanism called /broadcasting/. With IPv6, broadcasting isn't
...
have to set the socket option SO_BROADCAST before you can send a
...
common ways:
 ...
 1. Send the data to a specific subnet's broadcast address. This is the
    subnet's network number with all one-bits set for the host portion
    of the address. For instance, at home my network is 192.168.1.0, my
    netmask is 255.255.255.0, so the last byte of the address is my host
    number (because the first three bytes, according to the netmask, are
    the network number). So my broadcast address is 192.168.1.255. Under
    Unix, the *ifconfig* command will actually give you all this data.
    (If you're curious, the bitwise logic to get your broadcast address
    is /network_number/ OR (NOT /netmask/).) You can send this type of
    broadcast packet to remote networks as well as your local network,
    but you run the risk of the packet being dropped by the
 ...
 2. Send the data to the "global" broadcast address. This is
    ...
    automatically bitwise AND this with your network number to convert
    it to a network broadcast address, but some won't. It varies.
    Routers do not forward this type of broadcast packet off your local
    network, ironically enough.
...
In fact, that's the /only difference/ between a UDP application that can
broadcast and one that can't. So let's take the old *talker* application
and add one section that sets the SO_BROADCAST socket option. We'll call
this program /broadcaster.c/
...
another. You should be now be able to do all those sends that failed, above.
...
broadcast address, it could be that you have a firewall on your local
machine that is blocking the packets. (Yes, Pat and Bapper, thank you
for realizing before I did that this is why my sample code wasn't
...
*Where can I get those header files?*
    ...
    building for Windows, you only need to #include <winsock.h>.

*What do I do when *bind()* reports "Address already in use"?*

    You have to use *setsockopt()* with the SO_REUSEADDR option on the
    listening socket. Check out the section on <#bind>*bind()* and the
    section on <#select>*select()* for an example.

*How do I get a list of open sockets on the system?*

    Use the *netstat*. Check the *man* page for full details, but you
    should get some good output just typing:
...
computer? Don't I need a network to write network programs?*
    ...
    network "device" that sits in the kernel and pretends to be a
    network card. (This is the interface listed as "lo" in the routing
    table.)
    ...
    or *client localhost* (since "localhost" is likely defined in your
    //etc/hosts/ file) and you'll have the client talking to the server
    without a network!
...
more about raw sockets and SOCK_RAW?*
    ...
    Stevens' UNIX Network Programming books <#books>. Also, look in the
    /ping// subdirectory in Stevens' UNIX Network Programming source
    code, available online <http://www.unpbook.com/src.html>.

*How do I change or shorten the timeout on a call to *connect()*?*

    Instead of giving you exactly the same answer that W. Richard
    Stevens would give you, I'll just refer you to /lib/connect_nonb.c/
    in the UNIX Network Programming source code
    <http://www.unpbook.com/src.html>.

    The gist of it is that you make a socket descriptor with *socket()*,
    set it to non-blocking <#blocking>, call *connect()*, and if all
    goes well *connect()* will return -1 immediately and /errno/ will be
    set to EINPROGRESS. Then you call *select()* <#select> with whatever
    timeout you want, passing the socket descriptor in both the read and
    write sets. If it doesn't timeout, it means the *connect()* call
    completed. At this point, you'll have to use *getsockopt()* with the
    SO_ERROR option to get the return value from the *connect()* call,
    ...
    Notice that this has the added benefit of allowing your program to
    do something else while it's connecting, too. You could, for
    example, set the timeout to something low, like 500 ms, and update
    an indicator onscreen each timeout, then call *select()* again. When
    you've called *select()* and timed-out, say, 20 times, you'll know
    it's time to give up on the connection.
...
try to compile!*
    ...
    Solaris/SunOS <#solaris> in the introduction for an example of how
    to do this.

*Why does *select()* keep falling out on a signal?*

    Signals tend to cause blocked system calls to return -1 with /errno/
    set to EINTR. When you set up a signal handler with *sigaction()*,
    you can set the flag SA_RESTART, which is supposed to restart the
...
*How can I implement a timeout on a call to *recv()*?*
    ...
    Use *select()* <#select>! It allows you to specify a timeout
    parameter for socket descriptors that you're looking to read from.
    Or, you could wrap the entire functionality in a single function,
    like this:
    ...
    return 0? Well, if you recall, a return value of 0 on a call to
    *recv()* means that the remote side closed the connection. So that
    return value is already spoken for, and -1 means "error", so I chose
    -2 as my timeout indicator.
...
socket?*
    ...
    project <http://www.openssl.org/> for more info.)
    ...
    where the data is read and the data is sent (using *send()*) over
    the network, and stick some code in there that does the encryption.
...
executes them?*
    ...
    *close()*s the connection (that is, there are no subsequent system
    ...
    /Beware!/ Having the server execute what the client says is like
    ...
    what if the client sends "*rm -rf ~*"? It deletes everything in your
    account, that's what!

    So you get wise, and you prevent the client from using any except
    for a couple utilities that you know are safe, like the *foobar*
    utility:
    ...
    non-alphanumeric characters (including spaces, if appropriate) in
    the arguments for the command.

    As you can see, security is a pretty big issue when the server
    starts executing things the client sends.
...
receives all the data at the same time. What's going on?*
    ...
    the *sendall()* <#sendall> function implementation for details.)
    Once you're sure of that, then you need to call *recv()* in a loop
    until all your data is read.

    Read the section Son of Data Encapsulation <#sonofdataencap> for
    details on receiving complete packets of data using multiple calls
    to *recv()*.
...
kind of struct sigaction. What to do?*
    ...
    replace it with the Win32 equivalent: *CreateProcess()*. I don't
    know how to use *CreateProcess()*—it takes a bazillion arguments,
...
IP address so they can connect to my machine?*
    ...
    Unfortunately, the purpose of a firewall is to prevent people
    outside the firewall from connecting to machines inside the
    firewall, so allowing them to do so is basically considered a breach
    of security.
    ...
    masquerading or NAT or something like that. Just design your
    programs so that you're always the one initiating the connection,
    and you'll be fine.
    ...
    can forward to you either through it's NAT software, or through a
...
into promiscuous mode?*
    ...
    Ethernet-layer addresses here, not IP addresses--but since ethernet
    is lower-layer than IP, all IP addresses are effectively forwarded
    as well. See the section Low Level Nonsense and Network Theory
    <#lowlevel> for more info.)

    This is the basis for how a packet sniffer works. It puts the
    interface into promiscuous mode, then the OS gets every single
    packet that goes by on the wire. You'll have a socket of some type
    that you can read this data from.
    ...
    ioctl" you'll probably get somewhere. There's what looks like a
    decent writeup in Linux Journal
    <http://interactive.linuxjournal.com/article/4659>, as well.
...
"official" port numbers?*
    ...
    If the port is already in use, you'll get an "Address already in
    use" error when you try to *bind()*. Choose another port. (It's a
    good idea to allow the user of your software to specify an alternate
    port either with a config file or a command line switch.)

    There is a list of official port numbers
    <http://www.iana.org/assignments/port-numbers> maintained by the
    ...
    that is. All that matters is that no one else /on the same machine/
    is using that port when you want to use it.
...
setting it up for incoming connections with *listen()*, then you call
*accept()* to actually get yourself a new socket descriptor to use for
subsequent communication with the newly connected client.
...
/addr/ parameter. You can safely ignore it if you assume you're getting
a struct sockaddr_in back, which you know you are, because that's the
type you passed in for /addr/.
...
Yay! Alternatively, you could set the O_NONBLOCK flag on the listening
socket using *fcntl()*, and then it will never block, choosing instead
to return -1 with /errno/ set to EWOULDBLOCK.
...
*socket()* <#socketman>, *getaddrinfo()* <#getaddrinfoman>, *listen()*
<#listenman>, struct sockaddr_in <#sockaddr_inman>
...
tell you, but keep in mind this is only if you're filling out the struct
sockaddr by hand; if not, use the results from *getaddrinfo()*, as per
above. In IPv4, the /sin_addr.s_addr/ field of the struct sockaddr_in
structure is set to INADDR_ANY. In IPv6, the /sin6_addr/ field of the
struct sockaddr_in6 structure is assigned into from the global variable
/in6addr_any/. Or, if you're declaring a new struct in6_addr, you can
initialize it to IN6ADDR_ANY_INIT.
...
is so you can put it in the /serv_addr/ parameter. You /can/ call
...
host, you can use *send()* and *recv()* as well as *sendto()* and
*recvfrom()*. If you want.
...
/anything else/ at all with the socket, you can *close()* it, and it'll
...
calls *send()*, it'll receive a signal SIGPIPE and send() will return -1
...
possible Windows will get angry... And you wouldn't like it when it's angry.
...
parameter can also be NULL if you're using the AI_PASSIVE flag (see below.)
...
can be found in the IANA Port List
<http://www.iana.org/assignments/port-numbers> or in your
//etc/services/ file.
...
together with the | operator.) Check your man page for the complete list
of flags.

AI_CANONNAME causes the /ai_canonname/ of the result to the filled out
with the host's canonical (real) name. AI_PASSIVE causes the result's IP
address to be filled out with INADDR_ANY (IPv4)or /in6addr_any/ (IPv6);
this causes a subsequent call to *bind()* to auto-fill the IP address of
the struct sockaddr with the address of the current host. That's
excellent for setting up a server when you don't want to hardcode the
address.
...
linked list of struct addrinfos, and you can go through this list to get
all the addresses that match what you passed in with the hints.
...
*getnameinfo()*!/ In particular, *gethostbyname()* doesn't work well
with IPv6.
...
*gethostbyname()* to get its IP address and store it in a struct in_addr.

Conversely, if you have a struct in_addr or a struct in6_addr, you can
use *gethostbyaddr()* to get the hostname back. *gethostbyaddr()* /is/
...
addresses—it's a general-purpose structure that's pretty easy to use for
our specific purposes once you see how.)
...
reverse of *gethostbyname()*. As for parameters, even though /addr/ is a
char*, you actually want to pass in a pointer to a struct in_addr. /len/
...
really an array of struct in_addr*s in disguise. The last array element
is NULL.
...
A commonly defined alias for /h_addr_list[0]/. If you just want any old
IP address for this host (yeah, they can have more than one) just use
this field.
...
or *hstrerror()*. These work just like the classic /errno/, *perror()*,
...
*gethostname()* <#gethostnameman>, /errno/ <#errnoman>, *perror()*
<#perrorman>, *strerror()* <#perrorman>, struct in_addr <#sockaddr_inman>
...
takes an already loaded struct sockaddr and does a name and service name
lookup on it. It replaces the old *gethostbyaddr()* and
...
cast) in the /sa/ parameter, and the length of that struct in the /salen/.
...
ones. NI_NOFQDN will cause the /host/ to only contain the host name, not
the whole domain name. NI_NAMEREQD will cause the function to fail if
the name cannot be found with a DNS lookup (if you don't specify this
...
to a server, you now have what is known as a /peer/. Your peer is simply
...
*gethostname()* <#gethostnameman>, *gethostbyname()*
<#gethostbynameman>, *gethostbyaddr()* <#gethostbynameman>
...
use these at run time to handle different errors in different ways.
...
Set the socket to be non-blocking. See the section on blocking
...
*recv()*'d on the socket, the signal SIGIO will be raised. This is rare
to see, and beyond the scope of the guide. And I think it's only
...
that's larger than a char.) The upshot of this is that if you *send()* a
two-byte short int from an Intel box to a Mac (before they became Intel
boxes, too, I mean), what one computer thinks is the number 1, the other
will think is the number 256, and vice-versa.
...
16-bit (2 byte, very likely short) numbers. 64-bit machines might have a
*htonll()* for 64-bit ints, but I've not seen it. You'll just have to
write your own.
...
another, and the penultimate letter shows what you're converting /to/.
...
(e.g. "192.168.5.10") and vice-versa. If you have an IP address passed
on the command line or something, this is the easiest way to get a
struct in_addr to *connect()* to, or whatever. If you need more power,
try some of the DNS functions like *gethostbyname()* or attempt a /coup
...
it's "Network To ASCII"—the "toa" suffix has an analogous friend in the
C library called *atoi()* which converts an ASCII string to an integer.)

The function *inet_aton()* is the opposite, converting from a
dots-and-numbers string into a in_addr_t (which is the type of the field
s_addr in your struct in_addr.)
...
a struct in_addr or struct in6_addr containing the address you wish to
convert to a string. Finally /dst/ and /size/ are the pointer to the
...
struct sockaddr_in6. In that case, the opposite funcion *inet_pton()* is
what you're after.
...
points to where the result should be stored, which is probably a struct
in_addr or struct in6_addr.
...
occur on them; this can be zero if the timeout occurred. Also returns -1
...
Once you have a socket up and connected, you can read incoming data from
the remote side using the *recv()* (for TCP SOCK_STREAM sockets) and
*recvfrom()* (for UDP SOCK_DGRAM sockets).
...
Additionally, the *recvfrom()* takes a struct sockaddr*, /from/ that
will tell you where the data came from, and will fill in /fromlen/ with
the size of struct sockaddr. (You must also initialize /fromlen/ to be
the size of /from/ or struct sockaddr.)
...
Receive Out of Band data. This is how to get data that has been sent to
...
this flag. This will tell you what's waiting in the buffer for when you
call *recv()* "for real" (i.e. /without/ the MSG_PEEK flag. It's like a
sneak preview into the next *recv()* call.
...
/len/ parameter. It will ignore your wishes in extreme circumstances,
however, like if a signal interrupts the call or if some error occurs or
if the remote side closes the connection, etc. Don't be mad with it.
...
you requested in the /len/ parameter), or -1 on error (and *errno* will
be set accordingly.)
...
*send()* data to them without blocking, or if some exception has occurred.
...
timeout, or when an event occurs, whichever is first. The struct timeval
has two fields: /tv_sec/ is the number of seconds, to which is added
...
this error if it occurs. See the *fcntl()* reference page <#fcntlman>
...
accordingly.) Also, the sets are modified to show which sockets are ready.
...
sent to and from the broadcast address. Does nothing—/NOTHING!!/—to TCP
...
your system. If there is no parameter to be passed, /optval/ can be NULL.
...
a char instead of an int, and is set to, for example, a character value
of '1' instead of an int value of 1. Again, check your own man pages for
more info with "*man setsockopt*" and "*man 7 socket*"!
...
Send as "out of band" data. TCP supports this, and it's a way to tell
the receiving system that this data has a higher priority than the
normal data. The receiver will receive the signal SIGURG and it can then
receive this data without first receiving all the rest of the normal
data in the queue.
...
See the section on blocking <#blocking> for more details.
...
but I still want to *recv()* data on it! Or vice-versa! How can I do this?
...
/domain/ describes what kind of socket you're interested in. This can,
believe me, be a wide variety of things, but since this is a socket
guide, it's going to be PF_INET for IPv4, and PF_INET6 for IPv6.

/type/

	

Also, the /type/ parameter can be a number of things, but you'll
probably be setting it to either SOCK_STREAM for reliable TCP sockets
(*send()*, *recv()*) or SOCK_DGRAM for unreliable fast UDP sockets
(*sendto()*, *recvfrom()*.)

(Another interesting socket type is SOCK_RAW which can be used to
...
Finally, the /protocol/ parameter tells which protocol to use with a
certain socket type. Like I've already said, for instance, SOCK_STREAM
uses TCP. Fortunately for you, when using SOCK_STREAM or SOCK_DGRAM, you
can just set the protocol to 0, and it'll use the proper protocol
automatically. Otherwise, you can use *getprotobyname()* to look up the
...
*accept()* <#acceptman>, *bind()* <#bindman>, *getaddrinfo()*
<#getaddrinfoman>, *listen()* <#listenman>
...
beginning structure as struct sockaddr, and you can freely cast the
pointer of one type to the other without any harm, except the possible
end of the universe.

Just kidding on that end-of-the-universe thing...if the universe does
end when you cast a struct sockaddr_in* to a struct sockaddr*, I promise
you it's pure coincidence and you shouldn't even worry about it.
...
sockaddr_in6*, or struct sockadd_storage* to that type with ease and safety.
...
claim must be set to zero. Other people don't claim anything about it
(the Linux documentation doesn't even mention it at all), and setting it
to zero doesn't seem to be actually necessary. So, if you feel like it,
set it to zero using *memset()*.
...
the following excellent books. I used to be an affiliate with a very
popular internet bookseller, but their new customer tracking system is
incompatible with a print document. As such, I get no more kickbacks. If
you feel compassion for my plight, paypal a donation to beej@beej.us
<mailto:beej@beej.us>. :-)

/Unix Network Programming, volumes 1-2/ by W. Richard Stevens. Published
by Prentice Hall. ISBNs for volumes 1-2: 0131411551
<http://beej.us/guide/url/unixnet1>, 0130810819
...
and III: 0131876716 <http://beej.us/guide/url/intertcp1>, 0130319961
<http://beej.us/guide/url/intertcp2>, 0130320714
<http://beej.us/guide/url/intertcp3>.

/TCP/IP Illustrated, volumes 1-3/ by W. Richard Stevens and Gary R.
Wright. Published by Addison Wesley. ISBNs for volumes 1, 2, and 3 (and
a 3-volume set): 0201633469 <http://beej.us/guide/url/tcpi1>, 020163354X
<http://beej.us/guide/url/tcpi2>, 0201634953
<http://beej.us/guide/url/tcpi3>, (0201776316
<http://beej.us/guide/url/tcpi123>).

/TCP/IP Network Administration/ by Craig Hunt. Published by O'Reilly &
Associates, Inc. ISBN 0596002971 <http://beej.us/guide/url/tcpna>.
...
/Intro to TCP/IP <http://pclt.cis.yale.edu/pclt/COMM/TCPIP.HTM>/

/TCP/IP FAQ <http://www.faqs.org/faqs/internet/tcp-ip/tcp-ip-faq/part1/>/

/The Winsock FAQ <http://tangentsoft.net/wskfaq/>/
...
/Serialization <http://en.wikipedia.org/wiki/Serialization>/ (packing
and unpacking data)


      10.3. RFCs

RFCs <http://www.rfc-editor.org/>—the real dirt! These are documents
that describe assigned numbers, programming APIs, and protocols that are
used on the Internet. I've included links to a few of them here for your
enjoyment, so grab a bucket of popcorn and put on your thinking cap:

/RFC 1 <http://tools.ietf.org/html/rfc1>/—The First RFC; this gives you
an idea of what the "Internet" was like just as it was coming to life,
and an insight into how it was being designed from the ground up. (This
RFC is completely obsolete, obviously!)

/RFC 768 <http://tools.ietf.org/html/rfc768>/—The User Datagram Protocol
(UDP)

/RFC 791 <http://tools.ietf.org/html/rfc791>/—The Internet Protocol (IP)

/RFC 793 <http://tools.ietf.org/html/rfc793>/—The Transmission Control
Protocol (TCP)

/RFC 854 <http://tools.ietf.org/html/rfc854>/—The Telnet Protocol

/RFC 959 <http://tools.ietf.org/html/rfc959>/—File Transfer Protocol (FTP)

/RFC 1350 <http://tools.ietf.org/html/rfc1350>/—The Trivial File
Transfer Protocol (TFTP)

/RFC 1459 <http://tools.ietf.org/html/rfc1459>/—Internet Relay Chat
Protocol (IRC)

/RFC 1918 <http://tools.ietf.org/html/rfc1918>/—Address Allocation for
Private Internets

/RFC 2131 <http://tools.ietf.org/html/rfc2131>/—Dynamic Host
Configuration Protocol (DHCP)

/RFC 2616 <http://tools.ietf.org/html/rfc2616>/—Hypertext Transfer
Protocol (HTTP)

/RFC 2821 <http://tools.ietf.org/html/rfc2821>/—Simple Mail Transfer
Protocol (SMTP)

/RFC 3330 <http://tools.ietf.org/html/rfc3330>/—Special-Use IPv4 Addresses

/RFC 3493 <http://tools.ietf.org/html/rfc3493>/—Basic Socket Interface
Extensions for IPv6

/RFC 3542 <http://tools.ietf.org/html/rfc3542>/—Advanced Sockets
Application Program Interface (API) for IPv6

/RFC 3849 <http://tools.ietf.org/html/rfc3849>/—IPv6 Address Prefix
Reserved for Documentation

/RFC 3920 <http://tools.ietf.org/html/rfc3920>/—Extensible Messaging and
Presence Protocol (XMPP)

/RFC 3977 <http://tools.ietf.org/html/rfc3977>/—Network News Transfer
Protocol (NNTP)

/RFC 4193 <http://tools.ietf.org/html/rfc4193>/—Unique Local IPv6
Unicast Addresses

/RFC 4506 <http://tools.ietf.org/html/rfc4506>/—External Data
...
------------------------------------------------------------------------
     10.x.x.x: 3.4.1 <#indexId434909-85>
     192.168.x.x: 3.4.1 <#indexId434909-86>

     255.255.255.255: 7.6 <#indexId434909-176>, 9.13 <#indexId434909-271>

     *accept()*: 5.5 <#indexId434909-109>, 5.6 <#indexId434909-111>, 9.1
<#indexId434909-223>
     Address already in use: 5.3 <#indexId434909-100>, 8.0
<#indexId434909-183>
     AF_INET: 3.3 <#indexId434909-70>, 5.2 <#indexId434909-93>, 8.0
<#indexId434909-208>
     AF_INET6: 3.3 <#indexId434909-71>
     asynchronous I/O: 9.11 <#indexId434909-257>

     Bapper: 7.6 <#indexId434909-180>
     *bind()*: 5.3 <#indexId434909-94>, 8.0 <#indexId434909-186>, 9.2
<#indexId434909-229>
          implicit: 5.3 <#indexId434909-103>, 5.4 <#indexId434909-107>
     blah blah blah: 2.2 <#indexId434909-56>
     blocking: 7.1 <#indexId434909-142>
     books: 10.1 <#indexId434909-330>
     broadcast: 7.6 <#indexId434909-173>
     byte ordering: 3.2 <#indexId434909-61>, 3.3 <#indexId434909-73>,
7.4 <#indexId434909-167>, 9.12 <#indexId434909-263>

     client:
          datagram: 6.3 <#indexId434909-140>
          stream: 6.2 <#indexId434909-137>
     client/server: 6.0 <#indexId434909-132>
     *close()*: 5.9 <#indexId434909-120>, 9.4 <#indexId434909-234>
     *closesocket()*: 1.5 <#indexId434909-11>, 5.9 <#indexId434909-124>,
9.4 <#indexId434909-238>
     compilers:
          *gcc*: 1.2 <#indexId434909-2>
     compression: 8.0 <#indexId434909-206>
     *connect()*: 2.1 <#indexId434909-37>, 5.3 <#indexId434909-97>, 5.3
<#indexId434909-104>, 5.4 <#indexId434909-105>, 9.3 <#indexId434909-232>
          on datagram sockets: 5.8 <#indexId434909-119>, 6.3
<#indexId434909-141>, 9.3 <#indexId434909-233>
     Connection refused: 6.2 <#indexId434909-138>
     *CreateProcess()*: 1.5 <#indexId434909-16>, 8.0 <#indexId434909-212>
     *CreateThread()*: 1.5 <#indexId434909-17>
     CSocket: 1.5 <#indexId434909-13>
     Cygwin: 1.5 <#indexId434909-7>

     data encapsulation: 2.2 <#indexId434909-46>, 7.3 <#indexId434909-162>
     DHCP: 10.3 <#indexId434909-341>
     disconnected network: see private network.
     DNS:
     domain name service: see DNS.
     donkeys: 7.3 <#indexId434909-161>

     EAGAIN: 7.1 <#indexId434909-145>, 7.1 <#indexId434909-147>, 9.21
<#indexId434909-314>
     email to Beej: 1.6 <#indexId434909-18>
     encryption: 8.0 <#indexId434909-203>
     EPIPE: 9.4 <#indexId434909-236>
     /errno/: 9.10 <#indexId434909-251>, 9.16 <#indexId434909-275>
     Ethernet: 2.2 <#indexId434909-52>
     EWOULDBLOCK: 7.1 <#indexId434909-146>, 7.1 <#indexId434909-148>,
9.1 <#indexId434909-228>
     Excalibur: 7.5 <#indexId434909-171>
     external data representation standard: see XDR.

     F_SETFL: 9.11 <#indexId434909-255>
     *fcntl()*: 7.1 <#indexId434909-144>, 9.1 <#indexId434909-227>, 9.11
<#indexId434909-254>
     *FD_CLR()*: 7.2 <#indexId434909-152>, 9.19 <#indexId434909-291>
     *FD_ISSET()*: 7.2 <#indexId434909-153>, 9.19 <#indexId434909-292>
     *FD_SET()*: 7.2 <#indexId434909-151>, 9.19 <#indexId434909-290>
     *FD_ZERO()*: 7.2 <#indexId434909-154>, 9.19 <#indexId434909-293>
     file descriptor: 2.0 <#indexId434909-22>
     firewall: 3.4.1 <#indexId434909-83>, 7.6 <#indexId434909-178>, 8.0
<#indexId434909-213>
          poking holes in: 8.0 <#indexId434909-214>
     footer: 2.2 <#indexId434909-48>
     *fork()*: 1.5 <#indexId434909-15>, 6.0 <#indexId434909-133>, 8.0
<#indexId434909-211>
     FTP: 10.3 <#indexId434909-338>

     *getaddrinfo()*: 3.3 <#indexId434909-68>, 4.0 <#indexId434909-89>,
5.1 <#indexId434909-91>
     *gethostbyaddr()*: 5.10 <#indexId434909-128>, 9.7 <#indexId434909-243>
     *gethostbyname()*: 5.11 <#indexId434909-130>, 9.6
<#indexId434909-240>, 9.7 <#indexId434909-242>
     *gethostname()*: 5.11 <#indexId434909-129>, 9.6 <#indexId434909-239>
     *getnameinfo()*: 4.0 <#indexId434909-90>, 5.10 <#indexId434909-127>
     *getpeername()*: 5.10 <#indexId434909-125>, 9.9 <#indexId434909-248>
     *getprotobyname()*: 9.23 <#indexId434909-326>
     *getsockopt()*: 9.20 <#indexId434909-295>
     *gettimeofday()*: 7.2 <#indexId434909-156>
     goat: 8.0 <#indexId434909-192>
     goto: 8.0 <#indexId434909-199>

     header: 2.2 <#indexId434909-47>
     header files: 8.0 <#indexId434909-181>
     *herror()*: 9.7 <#indexId434909-246>
     *hstrerror()*: 9.7 <#indexId434909-247>
     *htonl()*: 3.2 <#indexId434909-63>, 9.12 <#indexId434909-260>, 9.12
<#indexId434909-265>
     *htons()*: 3.2 <#indexId434909-62>, 3.3 <#indexId434909-74>, 7.4
<#indexId434909-166>, 9.12 <#indexId434909-259>, 9.12 <#indexId434909-264>
     HTTP: 10.3 <#indexId434909-342>
     HTTP protocol: 2.1 <#indexId434909-40>

     ICMP: 8.0 <#indexId434909-195>
     IEEE-754: 7.4 <#indexId434909-168>
     INADDR_ANY:
     INADDR_BROADCAST: 7.6 <#indexId434909-177>
     *inet_addr()*: 3.4 <#indexId434909-78>, 9.13 <#indexId434909-270>
     *inet_aton()*: 3.4 <#indexId434909-79>, 9.13 <#indexId434909-269>
     *inet_ntoa()*: 3.4 <#indexId434909-81>, 9.13 <#indexId434909-268>
     *inet_ntoa()*: 3.4 <#indexId434909-80>, 5.10 <#indexId434909-126>
     *inet_pton()*: 3.4 <#indexId434909-77>
     Internet Control Message Protocol: see ICMP.
     Internet protocol: see IP.
     Internet Relay Chat: see IRC.
     ioctl(): 8.0 <#indexId434909-217>
     IP: 2.1 <#indexId434909-42>, 2.2 <#indexId434909-51>, 3.0
<#indexId434909-57>, 3.4 <#indexId434909-76>, 5.3 <#indexId434909-98>,
5.8 <#indexId434909-115>, 5.11 <#indexId434909-131>, 10.3
<#indexId434909-335>
     IP address: 9.2 <#indexId434909-230>, 9.6 <#indexId434909-241>, 9.7
<#indexId434909-244>, 9.9 <#indexId434909-249>
     IPv4: 3.1 <#indexId434909-58>
     IPv6: 3.1 <#indexId434909-60>, 3.3 <#indexId434909-75>, 3.4.1
<#indexId434909-87>, 4.0 <#indexId434909-88>
     IRC: 7.4 <#indexId434909-165>, 10.3 <#indexId434909-340>
     ISO/OSI: 2.2 <#indexId434909-53>

     layered network model: see ISO/OSI.
     Linux: 1.5 <#indexId434909-6>
     *listen()*: 5.3 <#indexId434909-96>, 5.5 <#indexId434909-108>, 9.15
<#indexId434909-272>
          backlog: 5.5 <#indexId434909-110>
          with select(): 7.2 <#indexId434909-158>
     lo: see loopback device.
     localhost: 8.0 <#indexId434909-193>
     loopback device: 8.0 <#indexId434909-191>

     *man* pages: 9.0 <#indexId434909-222>
     Maximum Transmission Unit: see MTU.
     mirroring: 1.7 <#indexId434909-19>
     MSG_DONTROUTE: 9.21 <#indexId434909-312>
     MSG_DONTWAIT: 9.21 <#indexId434909-313>
     MSG_NOSIGNAL: 9.21 <#indexId434909-316>
     MSG_OOB: 9.18 <#indexId434909-283>, 9.21 <#indexId434909-309>
     MSG_PEEK: 9.18 <#indexId434909-286>
     MSG_WAITALL: 9.18 <#indexId434909-287>
     MTU: 8.0 <#indexId434909-210>

     NAT: 3.4.1 <#indexId434909-84>
     *netstat*: 8.0 <#indexId434909-188>, 8.0 <#indexId434909-190>
     network address translation: see NAT.
     NNTP: 10.3 <#indexId434909-345>
     non-blocking sockets: 7.1 <#indexId434909-143>, 7.3
<#indexId434909-163>, 9.1 <#indexId434909-226>, 9.11
<#indexId434909-256>, 9.19 <#indexId434909-294>, 9.21 <#indexId434909-315>
     *ntohl()*: 3.2 <#indexId434909-65>, 9.12 <#indexId434909-262>, 9.12
<#indexId434909-267>
     *ntohs()*: 3.2 <#indexId434909-64>, 9.12 <#indexId434909-261>, 9.12
<#indexId434909-266>

     O_ASYNC: see asynchronous I/O.
     O_NONBLOCK: see non-blocking sockets.
     OpenSSL: 8.0 <#indexId434909-205>
     out-of-band data: 9.18 <#indexId434909-284>, 9.21 <#indexId434909-310>

     packet sniffer: 8.0 <#indexId434909-215>
     Pat: 7.6 <#indexId434909-179>
     *perror()*: 9.10 <#indexId434909-252>, 9.16 <#indexId434909-273>
     PF_INET: 8.0 <#indexId434909-207>, 9.23 <#indexId434909-320>
     ping: 8.0 <#indexId434909-194>
     *poll()*: 7.2 <#indexId434909-159>, 9.17 <#indexId434909-276>
     port: 5.8 <#indexId434909-116>, 9.2 <#indexId434909-231>, 9.9
<#indexId434909-250>
     ports: 5.3 <#indexId434909-95>, 5.3 <#indexId434909-99>
     private network: 3.4.1 <#indexId434909-82>
     promiscuous mode: 8.0 <#indexId434909-216>

     raw sockets: 2.1 <#indexId434909-33>, 8.0 <#indexId434909-196>
     *read()*: 2.0 <#indexId434909-27>
     *recv()*: 2.0 <#indexId434909-26>, 2.0 <#indexId434909-30>, 5.7
<#indexId434909-113>, 9.18 <#indexId434909-278>
          timeout: 8.0 <#indexId434909-200>
     *recvfrom()*: 5.8 <#indexId434909-117>, 9.18 <#indexId434909-279>
     *recvtimeout()*: 8.0 <#indexId434909-202>
     references: 10.1 <#indexId434909-331>
          web-based: 10.2 <#indexId434909-332>
     RFCs: 10.3 <#indexId434909-333>
     *route*: 8.0 <#indexId434909-189>

     SA_RESTART: 8.0 <#indexId434909-198>
     Secure Sockets Layer: see SSL.
     security: 8.0 <#indexId434909-209>
     *select()*: 1.5 <#indexId434909-12>, 7.1 <#indexId434909-149>, 7.2
<#indexId434909-150>, 8.0 <#indexId434909-187>, 8.0
<#indexId434909-201>, 9.19 <#indexId434909-288>
          with listen(): 7.2 <#indexId434909-157>
     *send()*: 2.0 <#indexId434909-25>, 2.0 <#indexId434909-29>, 2.2
<#indexId434909-54>, 5.7 <#indexId434909-112>, 9.21 <#indexId434909-305>
     *sendall()*: 7.3 <#indexId434909-160>, 7.5 <#indexId434909-170>
     *sendto()*: 2.2 <#indexId434909-55>, 9.21 <#indexId434909-306>
     serialization: 7.4 <#indexId434909-164>
     server:
          datagram: 6.3 <#indexId434909-139>
          stream: 6.1 <#indexId434909-134>
     *setsockopt()*: 5.3 <#indexId434909-101>, 7.6 <#indexId434909-174>,
8.0 <#indexId434909-184>, 8.0 <#indexId434909-221>, 9.20
<#indexId434909-296>
     *shutdown()*: 5.9 <#indexId434909-121>, 9.22 <#indexId434909-318>
     *sigaction()*: 6.1 <#indexId434909-135>, 8.0 <#indexId434909-197>
     SIGIO: 9.11 <#indexId434909-258>
     SIGPIPE: 9.4 <#indexId434909-235>, 9.21 <#indexId434909-317>
     SIGURG: 9.18 <#indexId434909-285>, 9.21 <#indexId434909-311>
     SMTP: 10.3 <#indexId434909-343>
     SO_BINDTODEVICE: 9.20 <#indexId434909-298>
     SO_BROADCAST: 7.6 <#indexId434909-175>, 9.20 <#indexId434909-300>
     SO_RCVTIMEO: 8.0 <#indexId434909-219>
     SO_REUSEADDR: 5.3 <#indexId434909-102>, 8.0 <#indexId434909-185>,
9.20 <#indexId434909-299>
     SO_SNDTIMEO: 8.0 <#indexId434909-220>
     SOCK_DGRAM: see socket;datagram.
     SOCK_RAW: 9.23 <#indexId434909-325>
     SOCK_STREAM: see socket;stream.
     socket: 2.0 <#indexId434909-21>
          datagram: 2.1 <#indexId434909-36>, 2.1 <#indexId434909-43>,
2.2 <#indexId434909-45>, 5.8 <#indexId434909-114>, 9.18
<#indexId434909-281>, 9.20 <#indexId434909-301>, 9.21
<#indexId434909-308>, 9.23 <#indexId434909-323>
          raw: 2.1 <#indexId434909-34>
          stream: 2.1 <#indexId434909-35>, 2.1 <#indexId434909-38>, 9.1
<#indexId434909-224>, 9.18 <#indexId434909-280>, 9.21
<#indexId434909-307>, 9.23 <#indexId434909-321>
          types: 2.0 <#indexId434909-31>, 2.1 <#indexId434909-32>
     socket descriptor: 2.0 <#indexId434909-24>, 3.3 <#indexId434909-66>
     *socket()*: 2.0 <#indexId434909-23>, 5.2 <#indexId434909-92>, 9.23
<#indexId434909-319>
     SOL_SOCKET: 9.20 <#indexId434909-297>
     Solaris: 1.4 <#indexId434909-3>, 9.20 <#indexId434909-303>
     SSL: 8.0 <#indexId434909-204>
     *strerror()*: 9.10 <#indexId434909-253>, 9.16 <#indexId434909-274>
     struct addrinfo: 3.3 <#indexId434909-67>
     struct hostent: 9.7 <#indexId434909-245>
     struct in_addr: 9.24 <#indexId434909-328>
     struct pollfd: 9.17 <#indexId434909-277>
     struct sockaddr: 3.3 <#indexId434909-69>, 5.8 <#indexId434909-118>,
9.18 <#indexId434909-282>, 9.24 <#indexId434909-329>
     struct sockaddr_in: 3.3 <#indexId434909-72>, 9.1
<#indexId434909-225>, 9.24 <#indexId434909-327>
     struct timeval: 7.2 <#indexId434909-155>, 9.19 <#indexId434909-289>
     SunOS: 1.4 <#indexId434909-4>, 9.20 <#indexId434909-302>

     TCP: 2.1 <#indexId434909-41>, 9.23 <#indexId434909-322>, 10.3
<#indexId434909-336>
     *gcc*: 2.1 <#indexId434909-39>, 10.3 <#indexId434909-337>
     TFTP: 2.2 <#indexId434909-49>, 10.3 <#indexId434909-339>
     timeout, setting: 8.0 <#indexId434909-218>
     translations: 1.8 <#indexId434909-20>
     transmission control protocol: see TCP.
     TRON: 5.4 <#indexId434909-106>

     UDP: 2.1 <#indexId434909-44>, 2.2 <#indexId434909-50>, 7.6
<#indexId434909-172>, 9.23 <#indexId434909-324>, 10.3 <#indexId434909-334>
     user datagram protocol: see UDP.

     Vint Cerf: 3.1 <#indexId434909-59>

     Windows: 1.5 <#indexId434909-5>, 5.9 <#indexId434909-122>, 8.0
<#indexId434909-182>, 9.4 <#indexId434909-237>, 9.20 <#indexId434909-304>
     Winsock: 1.5 <#indexId434909-8>, 5.9 <#indexId434909-123>
     Winsock FAQ: 1.5 <#indexId434909-14>
     *write()*: 2.0 <#indexId434909-28>
     *WSACleanup()*: 1.5 <#indexId434909-10>
     *WSAStartup()*: 1.5 <#indexId434909-9>

     XDR: 7.4 <#indexId434909-169>, 10.3 <#indexId434909-346>
     XMPP: 10.3 <#indexId434909-344>

     zombie process: 6.1 <#indexId434909-136>
